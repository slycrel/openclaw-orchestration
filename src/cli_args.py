"""Argument parser setup for the Maro orchestration CLI.
...
def build_parser() -> argparse.ArgumentParser:
    ...
    p_run = sub.add_parser("cycle", help="Run one orchestration cycle")
    ...
    p_memory = sub.add_parser("outcomes", help="Memory system — outcomes, lessons, session context (Phase 5)")
    ...
    p_director.add_argument("--model", "-m", help="LLM model string")
    p_director.add_argument("--dry-run", action="store_true", help="Simulate without API calls")
    p_director.add_argument("--verbose", "-v", action="store_true", help="Print progress")
    p_director.add_argument("--format", choices=["text", "json"], default="text", help="Output format")

    p_handle = sub.add_parser("handle", help="Send a request through handle (auto-routes NOW/AGENDA)")
    p_handle.add_argument("message", nargs="+", help="The request to handle")
    ...
    p_run = sub.add_parser("run", help="Run autonomous loop on a goal (Phase 1)")
    p_run.add_argument("goal", nargs="+", help="Goal description")
    p_run.add_argument("--project", "-p", help="Project slug (auto-created if not exists)")
    p_run.add_argument("--parent", help="Parent project slug (sets goal ancestry for this run)")
    p_run.add_argument("--parent-title", help="Human-readable title of the parent goal")
    p_run.add_argument("--model", "-m", help="LLM model string")
    p_run.add_argument("--max-steps", type=int, default=6, help="Max decomposition steps (default: 6)")
    p_run.add_argument("--max-iterations", type=int, default=20, help="Hard cap on LLM calls")
    ...
    p_evolver.add_argument("--window", type=int, default=50, help="How many recent outcomes to analyze (default: 50)")
    p_evolver.add_argument("--notify", action="store_true", help="Send Telegram summary of suggestions")
    ...
    p_metrics = sub.add_parser("metrics", help="Show quality + cost metrics (Phase 8/19)")
    p_metrics.add_argument("--format", choices=["text", "json"], default="text")
    metrics_sub = p_metrics.add_subparsers(dest="metrics_cmd")

    p_pass_k = metrics_sub.add_parser("pass-k", help="Show pass@k and pass^k for a skill (Phase 19)")
    ...
    p_eval = sub.add_parser("eval", help="Run evaluation benchmarks (Phase 8)")
    p_eval.add_argument("--dry-run", action="store_true", help="Return canned results without LLM calls")
    p_eval.add_argument("--benchmark", dest="benchmark_id", help="Run a specific benchmark by ID")
    p_eval.add_argument("--format", choices=["text", "json"], default="text")

    p_heartbeat = sub.add_parser("heartbeat", help="Run heartbeat — health check + tiered recovery (Phase 4)")
    p_heartbeat.add_argument("--loop", action="store_true", help="Run forever on an interval")
    p_heartbeat.add_argument("--interval", type=float, default=60.0, help="Seconds between checks (default: 60)")
    p_heartbeat.add_argument("--dry-run", action="store_true", help="Check only, no recovery or Telegram alerts")
    p_heartbeat.add_argument("--no-escalate", action="store_true", help="Skip Telegram escalation")
    autonomy_group = p_heartbeat.add_mutually_exclusive_group()
    ...
    p_heartbeat.add_argument("--backlog-every", type=int, default=None, help="Autonomous backlog drain cadence in heartbeat ticks (default: 5)")
    ...
    p_telegram.add_argument("--project", "-p", default="maro-telegram", help="Project slug for memory")
    p_telegram.add_argument("--verbose", "-v", action="store_true", default=True)
    ...
    p_mission = sub.add_parser("mission", help="Run a mission — milestone-gated multi-day goal pursuit (Phase 10)")
    p_mission.add_argument("goal", nargs="+", help="High-level multi-day goal")
    p_mission.add_argument("--project", "-p", help="Project slug (auto-created if not given)")
    p_mission.add_argument("--dry-run", action="store_true", help="Simulate without LLM calls")
    p_mission.add_argument("--verbose", "-v", action="store_true", help="Print progress")
    p_mission.add_argument("--format", choices=["text", "json"], default="text")

    p_mission_status = sub.add_parser("mission-status", help="Show mission status for a project or all projects (Phase 10)")
    p_mission_status.add_argument("project", nargs="?", help="Project slug (omit for all)")
    p_mission_status.add_argument("--format", choices=["text", "json"], default="text")

    p_background = sub.add_parser("background", help="Run a shell command in the background (Phase 10)")
    p_background.add_argument("command", nargs="+", help="Shell command to run")
    p_background.add_argument("--wait", action="store_true", help="Wait for completion (poll until done)")
    p_background.add_argument("--timeout", type=int, default=300, help="Timeout seconds (default: 300)")
    p_background.add_argument("--format", choices=["text", "json"], default="text")

    p_hooks = sub.add_parser("hooks", help="Manage hook registry (Phase 11)")
    ...
    p_inspector = sub.add_parser("inspector", help="Run quality inspector — friction detection + oversight (Phase 12)")
    p_inspector.add_argument("--loop", action="store_true", help="Run forever on an interval (for systemd)")
    p_inspector.add_argument("--interval", type=float, default=3600.0, help="Seconds between runs (default: 3600)")
    p_inspector.add_argument("--limit", type=int, default=50, help="Number of outcomes to inspect (default: 50)")
    p_inspector.add_argument("--dry-run", action="store_true", help="Run without LLM calls or saving results")
    p_inspector.add_argument("--format", choices=["text", "json"], default="text")
    ...
    p_ceo = sub.add_parser("conductor", help="Send a request through the Conductor (top-level orchestration role)")
    p_ceo.add_argument("message", nargs="+", help="The request to handle")
    p_ceo.add_argument("--model", "-m", help="LLM model string override")
    p_ceo.add_argument("--dry-run", action="store_true", help="Simulate without API calls")
    p_ceo.add_argument("--format", choices=["text", "json"], default="text")

    p_status = sub.add_parser("status", help="Executive summary (Phase 13)")
    p_status.add_argument("--format", choices=["text", "json"], default="text")
    p_status.add_argument("--dry-run", action="store_true", help="Simulate without API calls")

    p_map = sub.add_parser("map", help="Goal relationship map (Phase 13)")
    p_map.add_argument("--format", choices=["text", "json"], default="text")

    p_autonomy = sub.add_parser("autonomy", help="View/set autonomy tier config (Phase 13)")
    p_autonomy.add_argument("--tier", choices=["manual", "safe", "full"],
                                 help="Set default tier (or project/action tier)")
    p_autonomy.add_argument("--project", help="Project to set tier for")
    p_autonomy.add_argument("--action", dest="action_type",
                                 help="Action type to set tier for")
    p_autonomy.add_argument("--format", choices=["text", "json"], default="text")

    p_skills = sub.add_parser("skills", help="Manage skill library (Phase 10)")
    p_skills.add_argument("--extract", action="store_true", help="Extract skills from recent outcomes")
    p_skills.add_argument("--list", action="store_true", dest="list_skills", help="List known skills")
    p_skills.add_argument("--status", action="store_true", help="Show skill health dashboard (Phase 32)")
    p_skills.add_argument("--outcomes-window", type=int, default=50, help="How many recent outcomes to analyze (default: 50)")
    p_skills.add_argument("--dry-run", action="store_true", help="Analyze without writing skills")
    p_skills.add_argument("--format", choices=["text", "json"], default="text")
    p_skills.add_argument("--rollback", metavar="SKILL_NAME", help="Restore skills.jsonl from .bak (restores last-good state before evolver mutation)")

    # Phase 14 CLI subcommands
    p_attribution = sub.add_parser("attribution", help="Run failure attribution on recent stuck outcomes (Phase 14)")
    p_attribution.add_argument("--batch", action="store_true", help="Attribute all recent stuck outcomes (default: yes)")
    p_attribution.add_argument("--limit", type=int, default=20, help="Max outcomes to analyze (default: 20)")
    p_attribution.add_argument("--format", choices=["text", "json"], default="text")

    p_skill_stats = sub.add_parser("skill-stats", help="Show per-skill success rates (Phase 14)")
    p_skill_stats.add_argument("--escalated", action="store_true", help="Filter to skills needing redesign (below threshold)")
    p_skill_stats.add_argument("--format", choices=["text", "json"], default="text")

    p_skill_test = sub.add_parser("skill-test", help="Run or generate tests for a skill (Phase 14)")
    p_skill_test.add_argument("skill_id", help="Skill ID to test")
    ...
    gateway_sub.add_parser("opstatus", help="Check if OpenClaw gateway is reachable")
    ...
    router_sub = p_router.add_subparsers(dest="router_cmd", required=True)
    ...
    p_contract = sub.add_parser("contract", help="Sprint contract negotiation + grading (Phase 19)")
    ...
    p_boot = sub.add_parser("boot", help="Run Worker boot protocol for a project (Phase 19)")
    p_boot.add_argument("project", help="Project slug")
    p_boot.add_argument("--dry-run", action="store_true", help="Return minimal BootState without filesystem reads")
    p_boot.add_argument("--format", choices=["text", "json"], default="text")

    p_manifest = sub.add_parser("manifest", help="Show feature manifest status for a project (Phase 19)")
    p_manifest.add_argument("project", help="Project slug")
    p_manifest.add_argument("--format", choices=["text", "json"], default="text")

    p_memory = sub.add_parser("memory", help="Tiered memory management — short/medium/long tiers with decay (Phase 16)")
    memory_sub = p_memory.add_subparsers(dest="memory_cmd")
    memory_sub.add_parser("opstatus", help="Show tier breakdown, counts, decay candidates, promote candidates")
    ...
    p_persona = sub.add_parser("persona", help="Persona system — list, show, compose, and spawn agent personas (Phase 20)")
    ...
    p_knowledge = sub.add_parser("knowledge", help="Crystallization dashboard — view all knowledge graduation stages (Phase 22)")
    knowledge_sub = p_knowledge.add_subparsers(dest="knowledge_cmd")
