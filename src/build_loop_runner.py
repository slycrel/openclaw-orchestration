...
def _build_loop_session_timeout_seconds() -> float:
    raw = os.environ.get("MARO_BUILD_LOOP_SESSION_TIMEOUT_SECONDS", "").strip()
    if not raw:
...
def run_build_loop(
    *,
    ...
    if not any(os.environ.get(var) for var in ("MARO_ORCH_ROOT", "MARO_WORKSPACE", "OPENCLAW_WORKSPACE", "WORKSPACE_ROOT")):
        os.environ.setdefault("MARO_ORCH_ROOT", str(orch_root()))
    ...
    with _try_lock(build_loop_lock_path()) as acquired:
        ...
        try:
            with _interrupt_guard(), _temporary_env("MARO_YOLO", "true"):
                ticks = run_loop(
                    project=project,
                    worker=worker,
                    source="build-loop",
