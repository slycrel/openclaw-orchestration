...
0. **Substrate trial (OpenClaw → Maro → Telegram)** — *the current arc (Jeremy, 2026-07-01: "get the project where we can trial it for real with hermes or openclaw").* Substrate contract SHIPPED + live-verified 2026-07-01: notify hook (`notify.command`, run_completed/escalation, payload = run_card), uniform result retrieval (`run_result` + `maro-runs status|result`), escalation delivery (navigator dispatch-escalate + director surface both ping Telegram), `maro-notify-telegram` target, `deploy/openclaw/maro-dispatch.sh` symlinked into OpenClaw's scripts. E2E proven: dispatch script → enqueue → drain → run → run_card → Jeremy's Telegram DM (verified success-class goal). Docs: `docs/SUBSTRATE_INTEGRATION.md`. **Remaining:** (a) unattended hardening — per-run/daily budget cap verification, phantom `Step -1` rate-limit recovery (BACKLOG #2), drain-once semantics review; (b) OpenClaw-side delegation instruction (tell Poe when to hand a goal to `scripts/maro-dispatch.sh`); (c) burn-in: ~15 realistic goals through the full path, adjudicate via run_cards (done≠achieved + cost per goal); (d) Hermes adapter — deferred until after the OpenClaw trial, contract makes it cheap.
