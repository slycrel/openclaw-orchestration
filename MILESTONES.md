...
3. ~~**Director bypassed in practice**~~ — Session 20 finding 3.4 was partially hallucinated. `skip_if_simple` already defaults to `False`. Real gap (deferred to BACKLOG): NOW lane has no Director involvement at all by design. Architectural question — should complex NOW directives auto-escalate to Director? — punted as a separate decision.
