...
2. **Per-decision-class cutover** — *in progress 2026-06-12: dispatch-class cutover code shipped default-off (`navigator.act_dispatch` + confidence floor; escalate/close act, guard keeps first word; NAVIGATOR_ACTED audit event; `python3 -m navigator_shadow --agreement` is the evidence table). Enabling on this box waits on the organic batch in flight — at ship time: 19 live rows, 10/10 execute agreement, all 8 divergences adjudicated navigator-right or in-kind.* Closure class stays shadow-only (no live closure callsite yet).
