...
- [x] **Extract LoopStateMachine from agent_loop.py** — PARTIAL (2026-04-10). Phase 1 complete: LoopPhase constants + LoopContext dataclass + 10 extracted methods (_initialize_loop, _decompose_goal, _preflight_checks, _run_parallel_path, _prepare_execution, _handle_budget_ceiling, _select_step_adapter, _process_done_step, _build_result_and_finalize). run_agent_loop reduced from ~1,800 to ~1,100 lines. Remaining: while loop body (~800 lines of deeply coupled state) → natural next step is LoopStateMachine class where mutable state is self. Blocked-step processing (retry/split/terminal) stays inline due to flow control coupling.
