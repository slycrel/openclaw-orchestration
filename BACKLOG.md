...
- [ ] **Dynamic persona discovery** — persona system should be auto-discoverable, not hard-coded. Orchestrator sees a need, fills it with a new persona at runtime. Personas loaded from `personas/` directory scan rather than a static routing table. Orchestrator-generated personas: if a goal doesn't match any existing persona well, Poe proposes + writes a new persona spec and retries with it. (deferred)
