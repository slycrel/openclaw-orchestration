...
- [~] **HIGH: Director bypassed in practice** — Adversarial finding 3.4 was partially hallucinated. `skip_if_simple` defaults to `False` in `director.py:313` (NOT True as the review claimed); only `telegram_listener.py:306` sets True for `/director`/`/build`/`/ops` slash commands. The deeper observation is true in spirit: NOW-lane (`handle._run_now`) is a single LLM call with no Director involvement at all — by design. Real architectural question (deferred): should complex NOW directives optionally route through Director? Add config flag `now_lane.escalate_to_director` + `_is_complex_directive` heuristic + new code path in `handle()` that calls `run_director` when classifier returns NOW but complexity > threshold. Bigger lift; revisit when the routine NOW-lane workload demands it.
...
- [~] **MODERATE: Memory Stage 2→3 and 3→4 not implemented** — Stage 2→3 PARTIALLY FIXED (session 21): evolver now scans canon candidates + surfaces as crystallization Suggestions (human-gated, never auto-applied). Stage 3→4: extract_skills() exists in agent_loop finalize but wrapped in silent except; verify extraction actually works in live runs.
