...
- [ ] **Run-dir as the write destination (not a copy target).** Create `~/.poe/workspace/runs/<handle_id>-<nickname>/` at handle.py start. Wire existing artifact writers (agent_loop scratchpad, scope.py, resolved_intent emitter, planner, step output capture, captain's log slicer) to write there directly. Either (a) thread `run_dir` through the call sites, (b) use a process-level context var, or (c) symlink `project_workspace/artifacts/` into the run-dir. Pick whichever has the smallest blast radius. The point is no end-of-run gather step.
- [ ] **Run nickname module.** Deterministic 2-word nickname derived from handle_id (~50 adjectives × ~50 nouns ≈ 2500 combinations — unique-enough for local use, memorable, greppable). One small module, used by handle.py and any tooling that prints run identity.
- [ ] **Per-run repo bundle.** At end of every paid run, capture `git bundle create repo.bundle --all` + `git log --all --graph --oneline` + `git diff <base>..HEAD` into the run's artifact dir. Survives downstream resets, restores cleanly with `git clone repo.bundle`. (This one *is* an end-of-run capture by nature — git state isn't a stream of writes — but it lands in the same run-dir as everything else.)
