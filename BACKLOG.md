...
- [ ] **`poe evolver apply` CLI** — human-in-loop command to review and apply pending suggestions from suggestions.jsonl. Currently evolver applies automatically; manual gate needed for early adoption / high-stakes suggestions. Quick win: add `--apply-pending [--dry-run]` to evolver CLI or new `poe-evolver` command.
