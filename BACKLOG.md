...
- [ ] **Early model escalation on wide-scope goals** — Quality gate escalation (handle.py:618) waits for the full cheap run to complete before re-running at higher tier. This burns the entire cheap budget on goals that are clearly too complex. Two options: (a) pre-flight scope=wide → start at mid directly; (b) mid-loop quality checkpoint after step 2-3 → escalate early if output quality is low. Option (a) is simpler and cheaper. The pre-flight already computes scope — just wire it to model selection.
