...
def test_process_message_routes_to_handle():
    # Phase 13: natural language messages now route through conduct first.
    # Patch conduct to return a known response.
    # Also patch is_loop_running to False — parallel test workers can leave loop state set,
    # causing interrupt routing instead of conduct routing.
    bot = MagicMock()
    with patch("telegram_listener.conduct") as mock_poe, \
         patch("telegram_listener.is_loop_running", return_value=False):
        from conductor import ConductorResponse
        mock_poe.return_value = ConductorResponse(message="great answer", routed_to="now_lane")
...
def test_process_message_handle_error_sends_error():
    # Both conduct and handle raise — should fall through to error response
    bot = MagicMock()
    with patch("telegram_listener.conduct", side_effect=RuntimeError("poe boom")), \
...
def test_dispatch_slash_status():
    # Phase 13: /status routes through conduct for executive summary.
    # Patch conduct to return a predictable response.
    from conductor import ConductorResponse
    with patch("telegram_listener.conduct") as mock_poe:
        mock_poe.return_value = ConductorResponse(
...
def test_process_message_sends_ack_for_long_message():
    ...
    # Phase 13: conduct is called first for natural language.
    ...
    with patch("telegram_listener.conduct") as mock_poe, \
         patch("telegram_listener.is_loop_running", return_value=False):
        mock_poe.return_value = ConductorResponse(message="detailed answer here", routed_to="now_lane")
        _process_message(
            bot,
...
def test_process_message_short_message_no_ack():
    ...
    with patch("telegram_listener.conduct") as mock_poe, \
         patch("telegram_listener.is_loop_running", return_value=False):
        mock_poe.return_value = ConductorResponse(message="hi", routed_to="now_lane")
        _process_message(
            bot,
